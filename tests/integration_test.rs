...
use opentelemetry::{
    global,
    trace::{SpanKind, Status, Tracer, TraceContextExt},
};
...
async fn setup_test_graph(harness: &TestHarness) -> Result<InstrumentedGraph, Box<dyn std::error::Error>> {
...
fn validate_db_span_attributes(span: &SpanData) {
    ...
    let has_db_system = span.attributes.iter().any(|kv| {
        kv.key.as_str() == DB_SYSTEM_NAME
    });
    ...
    assert!(has_operation_info, "Missing {} or {} attribute", DB_OPERATION_NAME, DB_QUERY_TEXT);
    
    // Check for server address attribute
    let has_server_address = span.attributes.iter().any(|kv| {
        kv.key.as_str() == SERVER_ADDRESS
    });
    
    assert!(has_server_address, "Missing {} attribute", SERVER_ADDRESS);
    
    // Check for database namespace
    let has_db_namespace = span.attributes.iter().any(|kv| {
        kv.key.as_str() == DB_NAMESPACE
    });
    
    assert!(has_db_namespace, "Missing {} attribute", DB_NAMESPACE);
    
    // Check span kind is CLIENT for database calls
    assert_eq!(span.span_kind, SpanKind::Client, "Database spans should have CLIENT span kind");
}
...
async fn test_instrumented_run_query() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let query = Query::new("CREATE (n:TestNode {name: $name}) RETURN n".to_string())
        .param("name", "test");
    ...
    let db_system = query_span.attributes.iter()
        .find(|kv| kv.key.as_str() == DB_SYSTEM_NAME)
        .map(|kv| kv.value.as_str());
    
    let db_system_str = db_system.as_ref().map(|s| s.as_ref());
    assert_eq!(db_system_str, Some("neo4j"), "{} should be 'neo4j'", DB_SYSTEM_NAME);
...
async fn test_instrumented_execute_query() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let query = Query::new("CREATE (n:TestNode {value: $value}) RETURN n".to_string())
        .param("value", 42);
...
async fn test_instrumented_transaction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_tx_commit = spans.iter().any(|s| 
        s.name == "neo4j.transaction.commit" || 
        s.name.contains("commit")
    );
...
async fn test_query_with_parameters() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!spans.is_empty(), "Expected spans to be created for parameterized query");
    ...
    assert!(has_query_info, "Expected query information in span attributes");
...
async fn test_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error_span = spans.iter().find(|span| {
    ...
    if let Some(span) = error_span {
        ...
        assert!(has_error_info || span.status == Status::error(""), "Error spans should have error information");
    }
...
async fn test_span_relationships() -> Result<(), Box<dyn std::error::Error>> {
    ...
    graph.run(Query::new("CREATE (n:TestNode {nested: true})".to_string())).await?;
